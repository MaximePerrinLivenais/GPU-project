                                 ...
                                 const int height, const size_t pitch)
{
    ...
    lbp_value |= (threadIdx.y > 0 && image[(y - 1) * width + x] >= pixel_value)
        << 7;
    lbp_value |= (threadIdx.y > 0 && threadIdx.x > 0
                  && image[(y - 1) * width + x - 1] >= pixel_value)
        << 6;
    lbp_value |= (threadIdx.x > 0 && image[y * width + x - 1] >= pixel_value)
        << 5;
    lbp_value |= (threadIdx.y < 15 && threadIdx.x > 0
                  && image[(y + 1) * width + x - 1] >= pixel_value)
        << 4;
    lbp_value |= (threadIdx.y < 15 && image[(y + 1) * width + x] >= pixel_value)
        << 3;
    lbp_value |= (threadIdx.y < 15 && threadIdx.x < 15
                  && image[(y + 1) * width + x + 1] >= pixel_value)
        << 2;
    lbp_value |= (threadIdx.x < 15 && image[y * width + x + 1] >= pixel_value)
        << 1;
...
__global__ void init_histo_kernel(const unsigned char* histo_tab,
                                  const size_t tiles_number, const size_t pitch)
{
    int x = blockDim.x * blockIdx.x + threadIdx.x;
    int y = blockDim.y * blockIdx.y + threadIdx.y;
                        ...
                        const size_t height)
{
    ...
    rc = cudaMallocPitch(&lbp_values, &lbp_pitch, 256 * sizeof(unsigned char),
                         tiles_number);
    if (rc)
    {
    ...
    lbp_value_kernel<<<lbp_dim_grid, lbp_dim_block>>>(cuda_image, lbp_values,
                                                      width, height, pitch);
    ...
    size_t histo_pitch;
    rc = cudaMallocPitch(&histo_tab, &histo_pitch, 256 * sizeof(unsigned char),
                         tiles_number);
    if (rc)
    {
        std::cout << "Could not allocate memory for lbp values buffer\n";
    ...
    dim3 histo_dim_block(bsize, bsize);
    dim3 histo_dim_grid(w, h);

    init_histo_kernel<<<histo_dim_grid, histo_dim_block>>>(histo_tab, pitch,
                                                           tiles_number);
    ...
    cudaMemcpy2D(output, 0, histo_tab, pitch, 256 * sizeof(unsigned char), 1,
    ...
    unsigned char* output = (unsigned char*) malloc(256 * sizeof(unsigned
    char)); if (output == NULL)
    {
        std::cout << "CRINGE\n";
        exit(EXIT_FAILURE);
    ...
    cudaMemcpy2D(output, 0, lbp_values, pitch, 256 * sizeof(unsigned char),
