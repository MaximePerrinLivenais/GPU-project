...
__global__ compute_nearest_neighbors(const int* histo_tab,
                                        ...
                                        const size_t tiles_number)
{
    ...
    if (x >= TILE_SIZE || y >= N_CLUSTERS || tile_index >= tiles_number)
        return;

     __shared__ int cluster_distances[N_CLUSTERS];
...
void k_nearest_neighbors(const int* histo_tab, const float* clusters, const size_t tiles_number)
{
    ...
    auto tile_byte_size = TILE_SIZE * sizeof(int);
    ...
    auto cluster_byte_size = TILE_SIZE * sizeof(float);

    rc = cudaMallocPitch(&cuda_clusters, &cuda_clusters_pitch, cluster_byte_size, N_CLUSTERS);
    if (rc)
    {
    ...
    rc = cudaMemcpy2D(cuda_clusters, cuda_clusters_pitch, clusters, cluster_byte_size,
                        cluster_byte_size, N_CLUSTERS, cudaMemcpyHostToDevice);
    if (rc)
    {
    ...
    dim3 block_dim(TILE_SIZE, N_CLUSTERS);
    ...
    int* output = malloc(sizeof(int) * tiles_number);
